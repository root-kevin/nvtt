<script>
  ...
  export let vision = null;
  export let mapLighting = true;
  export let mode = "token";
  export let readOnly = false;
  export let onUpdateVision = () => {};
  ...
  let local = normalizeVision(vision);
  let lastTokenId = null;

  $: isReadOnly = readOnly || mode === "template";
  $: showUnlimited = mapLighting && !Number.isFinite(local?.distance);

  $: if (tokenId !== lastTokenId) {
    local = normalizeVision(vision);
    lastTokenId = tokenId;
  }

  function normalizeVision(rawInput = {}) {
...
<div class="vision-section">
  <div class="card">
    ...
    {#if isReadOnly}
      <div class="instance-note">Instance-only</div>
    {/if}
    <label class="toggle">
      <input
        type="checkbox"
    ...
    <div class="hint">
      Explicit distance overrides all. Leave empty for Unlimited when global lighting is on.
    </div>
    {#if showUnlimited}
    ...
    <div class="field-row">
      ...
      <div class="field-control">
        <input
          ...
          disabled={!local.enabled || isReadOnly}
        ...
        <input
          ...
          placeholder={mapLighting ? "Unlimited" : ""}
          disabled={!local.enabled || isReadOnly}
          on:input={(e) => handleDistanceInput(e.target.value)}
  ...
  <div class="card">
    ...
    <div class="hint">
      Applies only when global lighting is off and no explicit distance is set.
    </div>
    <div class="field-row">
      <span class="field-label">Distance (ft)</span>
      <div class="field-control">
        <input
          ...
          disabled={!local.hasDarkvision || isReadOnly}
